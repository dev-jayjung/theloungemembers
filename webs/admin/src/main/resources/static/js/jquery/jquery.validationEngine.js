 ...
 */
 (function($) {
	...
	var methods = {
			...
			for (var i = 0, field_errors = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "maxSize":
						...
						break;
					case "maxSizeNonUtf8":
						errorMsg = methods._getErrorMessage(form, field, rules[i], rules, i, options, methods._maxSizeNonUtf8);
						break;
					case "min":
		...
		* @author snw
		*/
		_maxSizeNonUtf8: function(field, rules, i, options) {
			var max = rules[i + 1];
			var len = field.val().length;
		...
		*/
		_calculatePosition: function (field, promptElmt, options) {
            ...
            var fieldWidth  = field.outerWidth(); // width() 대신 outerWidth() 권장

            // 🎯 position() 대신 offset()을 사용하여 전체 문서(document) 기준 좌표 계산
            var fieldOffset = field.offset();
            var fieldLeft   = fieldOffset ? fieldOffset.left : 0;
            var fieldTop    = fieldOffset ? fieldOffset.top : 0;

            var fieldHeight  = field.outerHeight(); // height() 대신 outerHeight() 권장
			...
			}else if((field.is("input") && $(field).attr("type") == "radio") || (field.is("input") && $(field).attr("type") == "checkbox")){
